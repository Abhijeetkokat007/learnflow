...
const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between items-center fixed z-50 w-full nav-bg px-6 py-2 lg:px-10 shadow-md">
      <div className="flex items-center gap-1">
        <a href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <img src={Darklogo} className="h-16" alt="Logo" />
        </a>
        {/* <p className="text-[26px] font-extrabold text-black dark:text-white max-sm:hidden">Brand</p> */}
      </div>

      <div className="hidden md:flex justify-center items-center text-[20px] font-bold lg:flex gap-5 text-black dark:text-white">
        <div
      ...
      <div className="flex items-center gap-5">
        <button
          onClick={handleThemeSwitch}
          className="h-10 hidden lg:flex sm:flex rounded-lg p-2  hover:bg-gray-100 border w-20 dark:hover:bg-gray-700"
        >
          <svg
            className="fill-green-500 w-20 hidden dark:block theme-button-logo"
            fill="currentColor"
        ...
        <GiHamburgerMenu
          className="md:hidden lg:hidden h-8 w-8 cursor-pointer text-black dark:text-white"
          onClick={toggleMenu}
        />
      </div>

      {isMenuOpen && (
        <div className="absolute top-0 left-0 w-full h-screen bg-white dark:bg-black flex flex-col items-center justify-center text-[20px] font-bold text-black dark:text-white space-y-5 md:hidden lg:hidden">
          <IoClose
            className="h-8 w-8 cursor-pointer text-black dark:text-white absolute top-6 right-6"
            onClick={toggleMenu}
          />
          <div
            className="cursor-pointer hover:text-green-500"
